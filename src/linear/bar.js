...
export class Bar extends Core {
  ...
  draw(args) {
    ...
    const x = (d, i) => (vertical ? 0 : barSpaceing(d, i));
    const y = (d, i) => (vertical ? barSpaceing(d, i) : displayAreaHeight - heightScale(d[0]));
    const height = d => (vertical ? bandWidthScale.bandwidth() : heightScale(d[0]));
    const width = d => (vertical ? heightScale(d[0]) : bandWidthScale.bandwidth());
