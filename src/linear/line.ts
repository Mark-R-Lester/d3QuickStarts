...
export class Line  {
  ...
  drawLine(args: LineArgs) {
    ...
    const meta: any[] = []
    const xVals: number[] = range(0, displayAreaWidth, displayAreaWidth / data.length)
    const yVals: number[] = range(0, displayAreaHeight, displayAreaHeight / data.length)
    const coordinates: [number, number][] = data.map((d, i) => (vertical ? [d, yVals[i]] : [xVals[i], d]))
    const coordinatesMin:[number, number][] = data.map((d, i) => (vertical ? [0, yVals[i]] : [xVals[i], 0]))
    meta.push({ class: 'line', id: `line${uuidv4()}`, coordinates, coordinatesMin })

    let spacingScale: any
    let bandingAdjustment: number
    if (banded) {
      spacingScale = scaleBand()
        .domain(coordinates.map(coordinate => (vertical ? coordinate[1].toString() : coordinate[0].toString())))
        .range(vertical ? [displayAreaHeight, 0] : [0, displayAreaWidth])
        bandingAdjustment = spacingScale.bandwidth() / 2
    } else {
      spacingScale = scaleLinear()
        .domain([0, Math.max(...coordinates.map(d => (vertical ? d[1] : d[0])))])
        .range(vertical ? [displayAreaHeight, 0] : [0, displayAreaWidth])
        bandingAdjustment = 0
    }
    const dataScale = scaleLinear()
      .domain([min, max !== 0 ? max : Math.max(...coordinates.map(d => (vertical ? d[0] : d[1])))])
