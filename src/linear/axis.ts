...
import { scaleLinear, scalePoint, scaleBand, ScaleBand, ScalePoint, ScaleLinear} from 'd3-scale'
import { Canvas, CanvasConfigStrict } from '../d3QuickStart'
import { Selection } from 'd3-selection'
import { axisBottom, axisTop, axisLeft, axisRight, AxisScale, Axis } from 'd3-axis'
...
export class Axiss {
  ...
  draw(data: string[] | number[], topOrRight: boolean, banded: boolean, isX: boolean) {
    ...
    const applyScaleToAxis = (scale: AxisScale<string>): { axis: Axis<string>, percentRange: number[]} => {
      if (isX) {
        return {
    ...
    const range: Iterable<number> = isX ? [0, displayAreaWidth] : [displayAreaHeight, 0]
    if (banded) {
      ...
      if (data.some(d => typeof d === 'string')) {
        ...
        const linerScale = scaleLinear()
          ...
          .range(range)
        scale = scalePoint()
          .domain(toStrings(linerScale.domain()))
          .range(range)
        const result = applyScaleToAxis(scale)
        axis = result.axis
        percentRange = result.percentRange
